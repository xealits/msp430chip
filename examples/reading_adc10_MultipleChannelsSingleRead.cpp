...
/// Multiple channels and single conversion.
/// Tested on msp.
...
void ConfigureAdc(void) {
    ...
    ctr0::write<
        ...
        // you still set multiple conversions:
    ...
    ctr1::write<
        ...
        // but sequence of channels now:
        | ctr1::conversion_sequence::set(ctr1::conversion_sequence::SequenceOfChannels)
...
int main(void)
{
    ...
    while (1) {
        ...
        unsigned adc_sum = 0;
        unsigned adc_min = 1 << 10;
        // now these readings are the channels
        // so, some of them are maxed out
        for (unsigned read_i = 0; read_i < n_adc_reeadings; read_i++) {
            auto reading = adc_readings[read_i];
            adc_sum += reading;
            if (reading < adc_min) adc_min = reading;
        }
        unsigned adc_average = adc_sum / n_adc_reeadings;

        device::Port1::p_out::write(
            (adc_average < 800 ? (1 << board::LED_RED) : 0x0)
            |
            //(count_cycle & 0x1 == 0x1 ? (1 << board::LED_GREEN) : 0x0)
            (adc_min < 512 ? (1 << board::LED_GREEN) : 0x0)
        );
